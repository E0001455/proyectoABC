...
public interface  RespuestaTareaCampanaRepository extends JpaRepository<RespuestaTareaCampanaEntity, Long> {
	...
	@Query(value ="SELECT ID_RESPUESTA_TAREA_CAMPANA, FCREQUESTID  FROM SYNONMID_TTABCTRA_RESPUESTA_TAREA_CAMPANA TRTC WHERE TRUNC(TRTC.FDFECHACREACION) = TRUNC(SYSDATE) ORDER BY ID_RESPUESTA_TAREA_CAMPANA ", nativeQuery = true)
	public 	List<Object[]> findRequestId();
	
	@Query(value ="SELECT DISTINCT(TEPR.ID_RESPUESTA_TAREA_CAMPANA),TRTC.FCREQUESTID\r\n"
			+ "FROM SYNONMID_TTABCTRA_EXTENSION_PERFIL TEP \r\n"
			+ "		        INNER JOIN SYNONMID_TTABCTRA_BITACORA_EXTENSION_PERFIL TBEP \r\n"
			+ "                ON TBEP.ID_EXTENSION_PERFIL = TEP.ID_EXTENSION_PERFIL \r\n"
			+ "              INNER JOIN SYNONMID_TTABCTRA_EXTENSION_PERFIL_RESPUESTA TEPR\r\n"
			+ "              ON TEPR.ID_EXTENSION_PERFIL = TBEP.ID_EXTENSION_PERFIL\r\n"
			+ "              INNER JOIN SYNONMID_TTABCTRA_RESPUESTA_TAREA_CAMPANA TRTC\r\n"
	...
	@Query(value ="SELECT TLCR.ID_EXTENSION_PERFIL FROM SYNONMID_TTABCTRA_RESPUESTA_TAREA_CAMPANA TRTL "
			...
			+ "			                  INNER JOIN SYNONMID_TTABCTRA_BITACORA_EXTENSION_PERFIL TBLC "
